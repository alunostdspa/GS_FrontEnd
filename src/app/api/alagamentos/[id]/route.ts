    ...
    try {
      ...
      if (response.ok) {
        ...
        return NextResponse.json(
          {
            message: errorData.message || "Erro ao buscar alagamento",
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return NextResponse.json(
          {
            message: errorData.message || "Erro ao atualizar alagamento",
            errors: errorData.errors || [errorData.message || "Erro desconhecido"],
          },
          { status: response.status },
        )
  ...
  try {
    ...
    try {
      ...
      if (response.ok || response.status === 204) {
        ...
        return NextResponse.json(
          {
            message: errorData.message || "Erro ao excluir alagamento",
            errors: errorData.errors || [errorData.message || "Erro desconhecido"],
          },
          { status: response.status },
        )
